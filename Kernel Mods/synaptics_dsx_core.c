...
/*
 * Sony SW3 Kernel modifications to add raw touch image reading functionality. 
 * The kernel we based this on is:
 * https://forum.xda-developers.com/smartwatch-3/orig-development/kernel-crpalmer-s-tetra-kernel-lca43-t3184872
 * Just swap in the synaptics_dsx_core.h and synaptics_dsx_core.c files. 
 
 * The code is based on:
 * 1. Raincheck project: https://github.com/isaaczinda/RainCheck
 * 2. patchwork.kernel.org: https://patchwork.kernel.org/patch/9235337/
 * 3. RMI4 utils: https://github.com/aduggan/rmi4utils
 * And many similar sources. 
 
 * The basic idea is to read the touchscreen raw data from the synaptic driver using the 
 * built in "Function 54". We were unable to find official documentation for this function. 
 * Words of warning: this modification disables regular touch screen messages - your touch 
 * screen will not work while you are running this kernel. The raw touch images are available
 * to regular applications on the watch via a proc file. 
 */
...
// IANIANIAN
bool usePixelImage    = true;  // true means use the pixel image
								// false means regular touch screen

// the below options only apply when usePixelImage is true
bool sendInterrupts   = false; 	// false means we don't try to send interrupts
								// manually during our polls.

bool useFastFrameWait = true;  	// true means we use a <framewait>ms (def 0ms) delay
								// between sending out pixel images. otherwise
								// its <initialWait> (def 500ms)

bool useBufferWait    = true; 	// true means we wait for the raw image buffer to
								// be marked full before reading the data. This
								// is correct but slow (~20-25ms). False means we
								// skip this to save time.... may work (YMMV).

int useFixedBufferWait= 10;     // use a fixed wait instead of a checking&wait for the buffer
								// set to zero if you don't want to use it.

bool sendDebugText    = false;  // whether or not we write dmesg msgs.

// Sony SW3 reports a 7x7 grid
// Can likely pull this from the device, but size parameters not clear/documented.
#define SKIP_ROWS 0 			// Had issues with initial rows of data on some devices. Watch is ok
#define TX 7					// == 7 + SKIP_ROWS // Rows
#define RX 7					// Cols

// Write the report type. I tested most of these (most are two byte)
// F54_8BIT_IMAGE      = 1 	// untried
// F54_16BIT_IMAGE     = 2 	// works well, but at ~30Hz
// --
// below is exactly what we'd expect
// the baseline vals change so slowly, we could probably read them once every
// few seconds if there is a benefit to reading the raw image.
// unfortunately I can't see any benenfit over F54_16BIT_IMAGE
// F54_RAW_16BIT_IMAGE = 3 	// works - high values (~3000) that change rapidly
// F54_TRUE_BASELINE   = 9 	// works - high values (~3000) that are pretty stable
// ---
// F54_FULL_RAW_CAP    = 19 // works - high values (~3000) that change rapidly. Refs are defined as min/max sensor vals
// F54_FULL_RAW_CAP_RX_OFFSET_REMOVED = 20 // works - high values ~ 5000. Not sure of meaning
#define REPORT_TYPE 2 // 16 bit image...
#define DATA_POINTS (RX * TX * 2)
//#define DATA_POINTS (RX * TX * 2) // Double size for report types except 1 (F54_8BIT_IMAGE)

#define PAGE_SELECT_REGISTER 0xFF
#define INDEX_OFFSET 1 //   correct from web
#define DATA_OFFSET 3  // 	correct from web
#define GET_REPORT_COMMAND 1

//these are the values that some of you need to change
#define F54_DATA_REGISTER 0x00
#define F54_COMMAND_REGISTER 0x6F // 111 in decimal
#define F54_QUERY_REGISTER 0x70	  // 112 in decimal

//this is the global client variable - interface to device
struct synaptics_rmi4_data *rmi4_data_global;
char DataBuffer[DATA_POINTS];
#define STRING_BUFFER_LENGTH (RX * TX * 7)
char StringBuffer[STRING_BUFFER_LENGTH]; // some guess at a max length for the string version

//this is multithreading stuff
static struct workqueue_struct *touch_img_reporting_workqueue;
static struct delayed_work Task;
int initialWait = 500; // half a second on boot and screen wake - don't advise changing
int frameWait   = 0;   // time between frames. Reads take some time (10-20ms),
											 // so zero may be best choice here
...
//IANIANIAN
	TOUCH_INFO_MSG("IO: Found F%02x (page %d, entry %hu, query %d, command %d, data %d\n", rmi_fd.fn_number, page_number, pdt_entry_addr, rmi_fd.query_base_addr, rmi_fd.cmd_base_addr, rmi_fd.data_base_addr);
//IANIANIAN
...
// IANIANIAN
void get_electrode_dims(struct synaptics_rmi4_data *rmi4_data)
	{
	char buffer[2];
	unsigned char pageData;
	int PageSelectRegister = 0xFF;
  int QueryRegister = 0x70;

	//switch to page 1
	pageData = 1;
  synaptics_rmi4_reg_write(rmi4_data, PageSelectRegister, &pageData, sizeof(pageData));

	// read the buffer
	synaptics_rmi4_reg_read(rmi4_data, QueryRegister, buffer, sizeof(buffer));
...
static void print_image_report(int reps)
	{
		// index
		int i;
		...
		// print out the packetCounter and rep count
		int position = snprintf(StringBuffer, STRING_BUFFER_LENGTH, "%i,%i;", packetCounter, reps);
		...
		// print out the ints to the string
		for (i = 0; i < (RX * TX); i++)
			{
			position += snprintf(StringBuffer + position, STRING_BUFFER_LENGTH-position, (i!=0 ? ",%hi" : "%hi"), *(data+i));
			if (position>=STRING_BUFFER_LENGTH)
				break;
			}

		// output debug msg
		if (sendDebugText) TOUCH_INFO_MSG("%s\n", StringBuffer);
	    ...
	    unsigned int Position = 2; // in the string
	    int i;
	    short TempShort;
	    char TempString[8];
	    int StringLength;

	    StringBuffer[0] = packetCounter;
	    StringBuffer[1] = ',';
	    packetCounter++;
	    if (packetCounter>'9')
				packetCounter='0';

			sprintf(TempString, "%i,", reps);
	    ...
	    StringBuffer[Position] = '[';
			...
			// First three rows are non sensors, so start afterwards
	    for (i = RX*SKIP_ROWS; i < (RX * TX); i++)
	    {
	        TempShort = *Data;
...
static void get_image_data(struct work_struct *work)
	...
	// for checking the command is ready
	unsigned char commRet;

	//setup report type
	unsigned char ReportType;

	// setup report command
	unsigned char ReportCommand;

	// the pageData for the calls - F54 is page 1, regular ops are page 0
	unsigned char pageData;

	int count = 0;

	if (sendDebugText)
		TOUCH_INFO_MSG("Getting touch image data %d\n", 1);

	//switch to page 1
	pageData = 1;
	Return = synaptics_rmi4_reg_write(rmi4_data_global, PAGE_SELECT_REGISTER, &pageData, sizeof(pageData));
	if (Return < 0) {if (sendDebugText) TOUCH_INFO_MSG("IntLab: failed on setting to page 1 (%d)", 0); return;}
	...
	Return = synaptics_rmi4_reg_write(rmi4_data_global, F54_DATA_REGISTER + INDEX_OFFSET, report_index, sizeof(report_index));
	if (Return < 0) {if (sendDebugText) TOUCH_INFO_MSG("IntLab: failed on writing report index(%d)", 1); return;}

	// choose a report type
	ReportType = REPORT_TYPE; // F54_16BIT_IMAGE==2
	Return = synaptics_rmi4_reg_write(rmi4_data_global, F54_DATA_REGISTER, &ReportType, sizeof(ReportType));
	if (Return < 0) {if (sendDebugText) TOUCH_INFO_MSG("IntLab: failed on setting report type (%d)", 2); return;}

	//write the get report command
	// GET_REPORT_COMMAND = 1;
	ReportCommand = GET_REPORT_COMMAND;
	Return = synaptics_rmi4_reg_write(rmi4_data_global, F54_COMMAND_REGISTER, &ReportCommand, sizeof(ReportCommand));
	if (Return < 0) {if (sendDebugText) TOUCH_INFO_MSG("IntLab: failed on writing get report command (%d)", 3); return;}

	// Wait until the command is completed
  if (useBufferWait) {
		if (sendDebugText) TOUCH_INFO_MSG("Start wait\n");
		commRet = 0x01;
		count = 0;
		do {
			// msleep(1); //wait 1ms // actually msleep doesn't work well at all for small vals (<20ms)
			usleep_range(500, 1000); // sleep between 0.5 and 1 ms.
			Return = synaptics_rmi4_reg_read(rmi4_data_global, F54_COMMAND_REGISTER, &commRet, sizeof(commRet));
			if (Return < 0) {if (sendDebugText) TOUCH_INFO_MSG("IntLab: failed on waiting for data (%d)", 4); return;}
			count++;
		} while (commRet != 0x00);
	} // here using a fixed wait period (does that work better?)
	else if (useFixedBufferWait>0 && useFixedBufferWait<25) // put a reasonable cap
		{
		if (sendDebugText) TOUCH_INFO_MSG("Start %dms fixed wait\n", useFixedBufferWait);
		usleep_range((useFixedBufferWait*1000)-500, (useFixedBufferWait*1000));
		}

	//read the data from the data register
	if (sendDebugText) TOUCH_INFO_MSG("End wait / Start read %d\n\n", count);
	Return = synaptics_rmi4_reg_read(rmi4_data_global, F54_DATA_REGISTER + DATA_OFFSET, DataBuffer, DATA_POINTS);
	if (sendDebugText) TOUCH_INFO_MSG("End read\n");
	if (Return < 0) {if (sendDebugText) TOUCH_INFO_MSG("IntLab: failed on data read (%d)", 5); return;}

	if (sendInterrupts)
	{
		// touch sensor only works correctly if we use the full data buffer
		// for the raw read. So otherwise, don't even bother
		// switch to page 0
		pageData = 0;
		Return = synaptics_rmi4_reg_write(rmi4_data_global, PAGE_SELECT_REGISTER, &pageData, sizeof(pageData));
		if (Return < 0) {if (sendDebugText) TOUCH_INFO_MSG("IntLab: failed on setting to page 0 (%d)", 6); return;}
		if (sendDebugText) TOUCH_INFO_MSG("IntLab: checking interrupts on page 0");

		// fake the irq checking thing...
		// Actually works, but (probably) generates too many events...
		// Or misses transient events (e.g. up/down)
		// Need build an app in the OS to check what these are
	  // touch_irq_handler_fake();
	}


	// send the data
	print_image_report(count);

	// queue the next read - change this to frameRate in the end (initial is slow, frame is fast)
	if (useFastFrameWait)
		queue_delayed_work(touch_img_reporting_workqueue, &Task, msecs_to_jiffies(frameWait));
	else
		queue_delayed_work(touch_img_reporting_workqueue, &Task, msecs_to_jiffies(initialWait));
	}
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	// IANIANIAN
	TOUCH_INFO_MSG("IO touch driver v5. Config is:\n");
	if (usePixelImage)
		TOUCH_INFO_MSG("1. Sending raw touch info\n");
	else
		TOUCH_INFO_MSG("1. Sending regular touch events\n");

  if (usePixelImage)
		{
		if (sendInterrupts)
			TOUCH_INFO_MSG("2. Sending regular touch events via touch interrupts (untested)\n");
		else
			TOUCH_INFO_MSG("2. Not sending regular touch events (stable)\n");

		if (useFastFrameWait)
			TOUCH_INFO_MSG("3. using a short pause between buffer reads of %d\n", frameWait);
		else
			TOUCH_INFO_MSG("3. using a long pause between buffer reads of %d\n", initialWait);

		if (useBufferWait)
			{
			TOUCH_INFO_MSG("4. Waiting for the raw touch image buffer to fill (stable, may be required)\n");
			TOUCH_INFO_MSG("5. NOT waiting a fixed pause for the raw touch image buffer to fill\n");
			}
		else
			{
			TOUCH_INFO_MSG("4. NOT waiting for the raw touch image buffer to fill (experimental, may be unstable)\n");
			if (useFixedBufferWait>0 && useFixedBufferWait<25)
				TOUCH_INFO_MSG("5. Using a fixed %d pause to wait for the raw touch image buffer to fill\n", useFixedBufferWait);
			else
				TOUCH_INFO_MSG("5. NOT waiting a fixed pause for the raw touch image buffer to fill\n");
			}


		if (sendDebugText)  // whether or not we write dmesg msgs.
			TOUCH_INFO_MSG("6. Outputing debug messages for every packet\n");
		else
			TOUCH_INFO_MSG("6. No regular debug mesages\n");
		}

	// make a global copy we can easily access in our workqueue
	rmi4_data_global = rmi4_data; // does this work? Seems to.

	// test of the funcitons and global version of the rmi4_data
  // get_electrode_dims(rmi4_data_global);

	//setup multithreading
	touch_img_reporting_workqueue = create_singlethread_workqueue("touch_img_reporting_wq");
	...
	if (usePixelImage)
		{
			queue_delayed_work(touch_img_reporting_workqueue, &Task, msecs_to_jiffies(initialWait));
			TOUCH_INFO_MSG("%s", "Starting work_queue to log touch pixel data");
		}
	// IANIANIAN
...
// IANIANIAN
// proc file
static int touch_img_proc_show(struct seq_file *m, void *v)
{
	seq_printf(m, "%s", StringBuffer);

	return 0;
}

static int touch_img_proc_open(struct inode *inode, struct file *file)
{
    return single_open(file, touch_img_proc_show, NULL);
}

static const struct file_operations touch_img_proc_fops = {
    .owner      = THIS_MODULE,
    .open       = touch_img_proc_open,
    .read       = seq_read,
    .llseek     = seq_lseek,
...
static int __init synaptics_rmi4_init(void)
{
	...
	proc_create("touch_img_reporting", 0, NULL, &touch_img_proc_fops);
...
static void __exit synaptics_rmi4_exit(void)
{
	...
	// IANIANIAN
	remove_proc_entry("touch_img_reporting", NULL); // add later for proc file work

	cancel_delayed_work(&Task);
	flush_workqueue(touch_img_reporting_workqueue);
	destroy_workqueue(touch_img_reporting_workqueue);
	// IANIANIAN
